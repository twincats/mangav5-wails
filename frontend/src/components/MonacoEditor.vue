...
function applyJsonSchemaOptions() {
  ...
  if (!props.jsonValidate && !props.jsonSchema) return
  const jsonDefaults = (monaco.languages as any).json?.jsonDefaults
  if (!jsonDefaults) return

  if (!props.jsonSchema) {
    jsonDefaults.setDiagnosticsOptions({
      validate: Boolean(props.jsonValidate),
    })
    return
  }

  const schemaUri = props.jsonSchemaUri ?? 'inmemory://schema/schema.json'
  // If modelUri is set, use it for file matching
  const fileMatch =
    props.jsonSchemaFileMatch ?? (modelUri ? [modelUri.toString()] : [])
